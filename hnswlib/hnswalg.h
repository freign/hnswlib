...
namespace hnswlib
{
    ...
    public:
        ...
        std::vector<uint8_t*> neighbor_centroids;
        ...
        void extract_neigbor_centroids()
        {
            ...
            for (int i = 0; i < max_elements_; i++)
            {
                ...
                pq_dist->transpose(neighbor_centroids[i], size);
            }
        }
...
//                    if (candidate_id == 0) continue;
            ...
            BaseSearchStopCondition<dist_t> *stop_condition = nullptr) const
        {
            ...
            vl_type *visited_array = vl->mass;
            vl_type visited_array_tag = vl->curV;
            //__m512i* simd_registers = nullptr;
            ...
            if(config -> use_PQ)
            {
                dists = (float *)aligned_alloc(64, sizeof(float) * ((maxM0_ + 15) & (~15)));
            }
            // {
            ...
            while (!candidate_set.empty())
            {
                ...
                if (config->use_PQ)
                {
                    neighbors_centroid_ids = neighbor_centroids[current_node_id];
                    _mm_prefetch((const char *)neighbors_centroid_ids, _MM_HINT_T0);
                    //neighbors_centroid_ids -= pq_dist->m * pq_dist->nbits / 8;
                    
                    memset(dists, 0, sizeof(float) * ((maxM0_ + 15) & (~15)));
                    pq_dist->calc_dist_ultimate((uint8_t *)neighbors_centroid_ids, size, dists);
...
//                    if (candidate_id == 0) continue;
                    ...
                    /* else
                    {
                        _mm_prefetch(neighbors_centroid_ids + pq_dist->m * pq_dist->nbits / 8, _MM_HINT_T0);
                        _mm_prefetch(neighbors_centroid_ids + pq_dist->m * pq_dist->nbits / 8 + 64, _MM_HINT_T0);
                    } */
#endif
                    if (!(visited_array[candidate_id] == visited_array_tag))
                    {
                        ...
                        if (config->use_PQ)
                        {
                            ...
                            dist = dists[j - 1];
                            /* float real_dist = fstdistfunc_(data_point, currObj1, dist_func_param_);
                            cout << "pq dist: " << dist << " real dist: " << real_dist << "\n"; */
                        }
                        else
                        {
        ...
        searchKnn(const void *query_data, size_t k, BaseFilterFunctor *isIdAllowed = nullptr) const
        {
            ...
            if (config->use_multiple_ep)
            {
                while (config->eps.size())
                {
                    if (config->eps.top().first > 1.2 * curdist)
                        config->eps.pop();
                    else
                        break;
                }
        ...
        void get_extent_neighbors()
        {
            ...
            for (int cur = 0; cur < max_elements_; cur++)
            {
                ...
                for (auto &pr : neighbors)
                {
                    ...
                    if (extent_neighbors[cur].size() >= maxM0_ * 2)
                        break;
                }
            }
        }
